...
| :-: | :- | :- | :- |
|1|Распределение чанков|```db.chunks.aggregate([{ $match: { ns: "somedb.products" } },{ $group: { _id: "$shard", chunkCount: { $sum: 1 } } },{ $sort: { chunkCount: -1 } }])```|Подсчёт чанков на каждом шарде. Если количество чанков значительно превышает (например x2) пороговое значение(например, среднее значение на остальных шардах) - Алерт|
...
|7|Bytes In/Out||Если полученные значения значительно(на 50+%) превышают пороговое значение(например, среднее значение на остальных шардах) - Алерт|
