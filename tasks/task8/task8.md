...
| :-: | :- | :- | :- |
|1|Распределение чанков|```db.chunks.aggregate([{ $match: { ns: "somedb.products" } },{ $group: { _id: "$shard", chunkCount: { $sum: 1 } } },{ $sort: { chunkCount: -1 } }])```|Подсчёт чанков на каждом шарде. Если количество чанков значительно превышает (например x2) пороговое значение(например, среднее значение на остальных шардах), значит, данный шард может стать горячим.|
...
|3|Нагрузка на шард в реальном времени|```db.currentOp({"secs_running": { "$gt": 1 },"ns": "somedb.products"})```|Сравнение значений с остальными шардами, значительное(на 50+%) отклонение от среднего значения указывает, что шард может стать горячим|
